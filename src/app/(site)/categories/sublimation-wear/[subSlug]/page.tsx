...
export async function generateMetadata({
  ...
  if (!sub) return { title: "Sublimation Wear" };
  return {
    title: `${sub.name} — Sublimation Wear`,
    description: sub.description,
  };
}

export default async function SublimationWearSubcategoryPage({
  params,
}: {
  params: Promise<{ subSlug: string }>;
}) {
  ...
  const items = await getSiteSubcategoryProducts("sublimation-wear", sub.name);
    ...
    <>
      <div className="border-b border-white/10 bg-[#0a0a0a]">
        <div className="container-x flex items-center gap-1.5 py-4 text-xs uppercase tracking-widest text-white/45">
          ...
          <ChevronRight size={12} />
          <Link href="/categories/sublimation-wear" className="hover:text-accent">
            Sublimation Wear
          </Link>
          <ChevronRight size={12} />
      ...
      <section className="bg-[#0a0a0a] py-16 text-white md:py-24">
        <div className="container-x">
          ...
          <Link
            href="/categories/sublimation-wear"
