...
export const metadata: Metadata = {
  title: "Sublimation Wear",
  description:
    "Full-body custom sublimation sportswear. Unlimited colors, zero fade, endless design possibilities — engineered for champions.",
};

export default async function SublimationWearPage() {
  const categories = await getSiteCategories();
  const otherCategories = categories.filter((c) => c.slug !== "sublimation-wear");
  const subNames = await getSiteSubcategories("sublimation-wear");
    ...
    <>
      ...
      <div className="border-b border-white/10 bg-[#0a0a0a]">
        <div className="container-x flex items-center gap-1.5 py-4 text-xs uppercase tracking-widest text-white/45">
          ...
          <ChevronRight size={12} />
          <span className="text-white">Sublimation Wear</span>
        </div>
      </div>
        ...
        <div className="container-x relative">
          <Reveal>
            ...
            <p className="mt-6 max-w-2xl text-sm leading-relaxed text-white/65 sm:text-base">
              Full-body custom sublimation sportswear. Unlimited colors, zero
              fade, endless design possibilities — engineered for champions.
            </p>
            ...
            {subcategoryItems.map((s, i) =>
              s.slug ? (
                <StaggerItem key={s.slug}>
                  <Link
                    href={`/categories/sublimation-wear/${s.slug}`}
