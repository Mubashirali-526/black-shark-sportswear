...
export async function generateMetadata({
  ...
  if (!sub) return { title: "Activewear" };
  return {
    title: `${sub.name} — Activewear`,
    description: sub.description,
  };
}

export default async function ActivewearSubcategoryPage({
  params,
}: {
  params: Promise<{ subSlug: string }>;
}) {
  ...
  const items = await getSiteSubcategoryProducts("activewear", sub.name);
    ...
    <>
      <div className="border-b border-white/10 bg-[#0a0a0a]">
        <div className="container-x flex items-center gap-1.5 py-4 text-xs uppercase tracking-widest text-white/45">
          ...
          <ChevronRight size={12} />
          <Link href="/categories/activewear" className="hover:text-accent">
            Activewear
          </Link>
          <ChevronRight size={12} />
