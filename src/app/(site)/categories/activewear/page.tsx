...
export const metadata: Metadata = {
  title: "Activewear",
  description:
    "Premium performance activewear engineered for serious athletes. Studio-ready designs meet competition-grade fabric technology.",
};

export default async function ActivewearPage() {
  const categories = await getSiteCategories();
  const otherCategories = categories.filter((c) => c.slug !== "activewear");
  const subNames = await getSiteSubcategories("activewear");
    ...
    <>
      ...
      <div className="border-b border-white/10 bg-[#0a0a0a]">
        <div className="container-x flex items-center gap-1.5 py-4 text-xs uppercase tracking-widest text-white/45">
          ...
          <ChevronRight size={12} />
          <span className="text-white">Activewear</span>
        </div>
      </div>
      ...
      <section className="relative isolate flex min-h-[600px] items-center overflow-hidden bg-gradient-to-b from-[#0a0a0a] to-[#1a1a1a] py-20">
        <Image
          src="/categories/activewear/active-hero.png"
          alt=""
          fill
          priority
          sizes="100vw"
        ...
        <div className="container-x relative">
          <Reveal>
            ...
            <p className="mt-6 max-w-2xl text-sm leading-relaxed text-white/65 sm:text-base">
              Premium performance activewear engineered for serious athletes.
              Studio-ready designs meets competition-grade fabric technology.
            </p>
            ...
            {subcategoryItems.map((s, i) =>
              s.slug ? (
                <StaggerItem key={s.slug}>
                  <Link
                    href={`/categories/activewear/${s.slug}`}
