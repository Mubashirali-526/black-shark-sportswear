...
export async function generateMetadata({
  ...
  if (!sub) return { title: "Youth Sportswear" };
  return {
    title: `${sub.name} — Youth Sportswear`,
    description: sub.description,
  };
}

export default async function YouthSportswearSubcategoryPage({
  params,
}: {
  params: Promise<{ subSlug: string }>;
}) {
    ...
    <>
      <div className="border-b border-white/10 bg-[#0a0a0a]">
        <div className="container-x flex items-center gap-1.5 py-4 text-xs uppercase tracking-widest text-white/45">
          ...
          <ChevronRight size={12} />
          <Link href="/categories/youth-sportswear" className="hover:text-accent">
            Youth Sportswear
          </Link>
          <ChevronRight size={12} />
      ...
      <section className="bg-[#0a0a0a] py-16 text-white md:py-24">
        <div className="container-x max-w-2xl">
          <p className="text-base leading-relaxed text-white/70">{sub.description}</p>

          <div className="mt-10 rounded-2xl border border-white/10 bg-ink-soft p-8 text-center">
          ...
          <Link
            href="/categories/youth-sportswear"
