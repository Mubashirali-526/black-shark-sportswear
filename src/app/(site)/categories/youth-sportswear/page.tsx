...
export const metadata: Metadata = {
  title: "Youth Sportswear",
  description:
    "Durable, comfortable sportswear built for young athletes — engineered to keep up with the next generation of champions.",
};

export default async function YouthSportswearPage() {
  const categories = await getSiteCategories();
  const otherCategories = categories.filter((c) => c.slug !== "youth-sportswear");
  const subNames = await getSiteSubcategories("youth-sportswear");
    ...
    <>
      ...
      <div className="border-b border-white/10 bg-[#0a0a0a]">
        <div className="container-x flex items-center gap-1.5 py-4 text-xs uppercase tracking-widest text-white/45">
          ...
          <ChevronRight size={12} />
          <span className="text-white">Youth Sportswear</span>
        </div>
      </div>
        ...
        <div className="container-x relative">
          <Reveal>
            ...
            <p className="mt-6 max-w-2xl text-sm leading-relaxed text-white/65 sm:text-base">
              Durable, comfortable sportswear built for young athletes —
              engineered to keep up with the next generation of champions.
            </p>
            ...
            {subcategoryItems.map((s, i) =>
              s.slug ? (
                <StaggerItem key={s.slug}>
                  <Link
                    href={`/categories/youth-sportswear/${s.slug}`}
