...
export const metadata: Metadata = {
  title: "Team Uniforms",
  description:
    "Professional-grade sublimated kits, customizable team uniforms, and durable warmup gear manufactured to meet league regulatory standards.",
};

export default async function TeamUniformsPage() {
  const categories = await getSiteCategories();
  const otherCategories = categories.filter((c) => c.slug !== "team-uniforms");
  const subNames = await getSiteSubcategories("team-uniforms");
    ...
    <>
      ...
      <div className="border-b border-white/10 bg-[#0a0a0a]">
        <div className="container-x flex items-center gap-1.5 py-4 text-xs uppercase tracking-widest text-white/45">
          ...
          <ChevronRight size={12} />
          <span className="text-white">Team Uniforms</span>
        </div>
      </div>
      ...
      <section className="relative isolate flex min-h-[600px] items-center overflow-hidden bg-gradient-to-b from-[#0a0a0a] to-[#1a1a1a] py-20">
        <Image
          src="/categories/team-uniforms/teamwear-hero.png"
          alt=""
          fill
          priority
          sizes="100vw"
        ...
        <div className="container-x relative">
          <Reveal>
            ...
            <p className="mt-6 max-w-2xl text-sm leading-relaxed text-white/65 sm:text-base">
              Professional-grade sublimated kits, customizable team uniforms, and
              durable warmup gear manufactured to meet league regulatory standards.
            </p>
            ...
            {subcategoryItems.map((s, i) =>
              s.slug ? (
                <StaggerItem key={s.slug}>
                  <Link
                    href={`/categories/team-uniforms/${s.slug}`}
