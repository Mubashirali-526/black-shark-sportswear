...
export async function generateMetadata({
  ...
  if (!sub) return { title: "Team Uniforms" };
  return {
    title: `${sub.name} — Team Uniforms`,
    description: sub.description,
  };
}

export default async function TeamUniformsSubcategoryPage({
  params,
}: {
  params: Promise<{ subSlug: string }>;
}) {
  ...
  const items = await getSiteSubcategoryProducts("team-uniforms", sub.name);
    ...
    <>
      <div className="border-b border-white/10 bg-[#0a0a0a]">
        <div className="container-x flex items-center gap-1.5 py-4 text-xs uppercase tracking-widest text-white/45">
          ...
          <ChevronRight size={12} />
          <Link href="/categories/team-uniforms" className="hover:text-accent">Team Uniforms</Link>
          <ChevronRight size={12} />
      ...
      <section className="bg-[#0a0a0a] py-16 text-white md:py-24">
        <div className="container-x">
          ...
          <Link
            href="/categories/team-uniforms"
            className="mt-10 inline-flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide text-white/60 transition-colors hover:text-accent"
          >
            <ArrowLeft size={14} />
            Back to Team Uniforms
          </Link>
        </div>
      </section>
    </>
