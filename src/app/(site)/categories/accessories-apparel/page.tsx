...
export const metadata: Metadata = {
  ...
  description:
    "From custom caps and socks to hoodies, jackets and tees — complete your team look with premium branded accessories.",
};

export default async function AccessoriesApparelPage() {
  const categories = await getSiteCategories();
  const otherCategories = categories.filter((c) => c.slug !== "accessories-apparel");
  const subNames = await getSiteSubcategories("accessories-apparel");
    ...
    <>
      ...
      <div className="border-b border-white/10 bg-[#0a0a0a]">
        <div className="container-x flex items-center gap-1.5 py-4 text-xs uppercase tracking-widest text-white/45">
          ...
          <ChevronRight size={12} />
          <span className="text-white">Accessories &amp; Apparel</span>
        </div>
      </div>
        ...
        <div className="container-x relative">
          <Reveal>
            ...
            <p className="mt-6 max-w-2xl text-sm leading-relaxed text-white/65 sm:text-base">
              From custom caps and socks to hoodies, jackets and tees —
              complete your team look with premium branded accessories.
            </p>
            ...
            {subcategoryItems.map((s, i) =>
              s.slug ? (
                <StaggerItem key={s.slug}>
                  <Link
                    href={`/categories/accessories-apparel/${s.slug}`}
